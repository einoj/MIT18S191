...
random_vect = [rand() for i in 1:10]# replace this with your code!
...
function mean_colors(image)
	...
	return (red,green,blue)./length(image)
end
...
# ╔═╡ ddc6f110-f4d7-11ea-0020-bb72ef514655
quantize(RGB(0.3,0.5,0.1))
...
begin
function myclamp(x::ColorTypes.Gray{Float64})
		if x<0
			return 0
		elseif x>1
			return 1
	...
	function myclamp(x::ColorTypes.RGB{Float64})
		if x<0
			return 0
		elseif x>1
			return 1
...
@bind n Slider(10:300, show_value=true)
...
function blur_1D(v, l)
	ext_v = [extend(v,i) for i in 1-l:length(v)+l]
	ret_v = copy(v)
	for (i, val) in enumerate(v)
		ext_i = i+l
		ret_v[i] = mean(ext_v[ext_i-l:ext_i+l])
	end
	return ret_v
end

# ╔═╡ 2b2b6630-f5bd-11ea-1519-9742ba6fe10d
colored_line(blur_1D(v,2))

# ╔═╡ 49c3200e-f5bd-11ea-0814-df95e74546e3
colored_line(v)
...
# ╔═╡ ba8f7070-f5c0-11ea-3e58-dd121f8f417e
@bind l_box Slider(0:length(v), show_value=true)

# ╔═╡ 2cb6762e-f5c1-11ea-11eb-47c185a61322
...
function convolve_vector(v, k)
	...
	for (i, val) in enumerate(v)
		ext_i = i+l
		ret_v[i] = sum(ext_v[ext_i-l:ext_i+l].*k)
	end
	return ret_v
end
...
function gaussian_kernel(n)
	...
	return [(1/sqrt(2*pi))*exp((-x^2)/2) for x = 1:n]
end
...
@bind gaussian_kernel_size_1D Slider(1:10, show_value=true) # change this value, or turn me into a slider!

# ╔═╡ 02bb6dc0-f5cd-11ea-0e99-8b3fa98d4608
...
begin
l, h = size(small_image)
[extend_mat(small_image, i, j) for (i,j) in Iterators.product(-1:l+2,-1:l+2)]
end
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	rl = (kr - 1) ÷ 2
	cl = (kc - 1) ÷ 2
	ext_v = [extend_mat(M, i, j) for (i,j) in Iterators.product(1-kr:rows+kr,1-kc:cols+kc)]
	ret_v = copy(M)
	for i in 1+kr:rows+kr
		for j in 1+kc:cols+kc
			ret_v[i-kr,j-kc] = sum(ext_v[i-rl:i+rl,j-cl:j+cl].*K)
		end
	end
	return ret_v
end
...
# ╔═╡ 0d08a210-f669-11ea-0cb1-65119d644731
begin
	rows, cols = size(test_image_with_border)
kr, kc = size(K_test)
	ext_v = [extend_mat(test_image_with_border, i, j) for (i,j) in Iterators.product(1-kr:rows+kr,1-kc:cols+kc)]
	ext_v[1:1+kr-1,1:1+kc-1].*K_test
end
...
# ╔═╡ 38870580-f826-11ea-235c-c3d6cc83a66e
begin
philip_head =philip[250:430,110:230]
	[extend_mat(philip_head, i, j) for (i,j) in Iterators.product(-50:size(philip_head,1)+51, (-50:size(philip_head,2)+51))]
end

# ╔═╡ 59385090-f826-11ea-1010-8b9dee12ab0a
convolve_image(philip_head, K_test)

# ╔═╡ 833716a0-f827-11ea-0a8d-9f27dc7473d5
typeof(philip)

# ╔═╡ 22a1b580-f66e-11ea-0c7c-e9b3c00f4fcb
K_sharpen = [[-1/2,-1,-1/2]  [-1,7,-1]  [-1/2,-1,-1/2]] 

# ╔═╡ fd3149d0-f66f-11ea-3b1a-05e81b6a1ad0
convolve_image(philip_head, K_sharpen)
...
# ╔═╡ dfeade40-f813-11ea-3135-c97fe31b8edf
function gen_gaussian_kernel(dims, sigma)
	kernel = zeros(Float64, dims, dims)
	for x=1:dims,y=1:dims
		kernel[x,y] = exp(-(x^2+y^2)/(2*sigma^2))/(2*pi*sigma^2)*5
	end
	return kernel
end

# ╔═╡ 8b2e3920-f817-11ea-3610-fb0b38a0ed72
gen_gaussian_kernel(3,20)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	
	return convolve_image(image, gen_gaussian_kernel(9,5))
end

# ╔═╡ 872a60f0-f813-11ea-03b5-556d1371fcf2
with_gaussian_blur(Gray.(philip))

# ╔═╡ 0cbff5b0-f99d-11ea-2ea0-471b50f0639c
philip[100].b
...
with_gaussian_blur(gauss_raw_camera_data)
...
with_sobel_edge_detect(Gray.(sobel_camera_image))
